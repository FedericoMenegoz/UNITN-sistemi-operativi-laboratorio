...
// Opens a message queue with the given key. If the queue does not exist it will
// create it. It returns -1 if an error occured.
int open_queue(key_t key);

// Exits the program and sends a signal to the specified process.
void exit_and_signal(int exit_code);
...
// And signaling the process provided in the args.
...
int main(int argc, char *argv[]) {
    ...
    if (argc == 4) {
        ...
        // get
            ...
            // get message if any
            if (msgrcv(queueId, &msg_rcv, sizeof(msg_rcv.text), 0,
                       IPC_NOWAIT) == -1) {
                if (errno != ENOMSG) {
                    perr("Error getting the message from queue. msgrcv",
                         ERR_QUEUE);
                }
            } else {
                printf("%s\n", msg_rcv.text);
        ...
        // emp
            ...
            while (msgrcv(queueId, &msg_rcv, sizeof(msg_rcv.text), 0,
                          IPC_NOWAIT) != -1) {
                printf("%s\n", msg_rcv.text);
            }
        } else {
            printf(
                "\n%sWrong args. Specify a value only when action is 'put' or "
                "'move'.\n%s\n",
                RED, DF);
        ...
        // mov
            ...
            while (msgrcv(queueId, &msg_rcv, sizeof(msg_rcv.text), 0,
                ...
                if (msgsnd(queueId2, &msg_rcv, strlen(msg_rcv.text), 0) == -1) {
                    perr(
                        "\nError transferring the message to the destination "
                        "queue. "
                        "msgsnd",
                        ERR_QUEUE);
                }
