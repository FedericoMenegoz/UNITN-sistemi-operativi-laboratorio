...
int queue_fail = 0, fifo_fail = 0, signal_receiver_pid, program_pid, sig1_flag,
    sig2_flag;
...
void *queue_reader_routine(__attribute__((unused)) void *args) {
    ...
    for (int i = 0; i < messages && !queue_fail; i++) {
        ...
        if (strcmp(msg_rcv.text, WORDS[i]) != 0) {
            snprintf(msg, MAX_BUFFER, "Queue msg %s != %s", msg_rcv.text,
                     WORDS[i]);
            fail(msg);
            queue_fail = 1;
        }
        if (msg_rcv.category != i + 1) {
            snprintf(msg, MAX_LINE, "Queue cat %ld != %d", msg_rcv.category,
                     i + 1);
            fail(msg);
            queue_fail = 1;
        } else {
...
void *fifo_reader_routine(__attribute__((unused)) void *args) {
    ...
    for (int i = 0; i < messages && !fifo_fail; i++) {
        ...
        if (strcmp(fifo_buffer, WORDS[i]) != 0) {
            snprintf(fifo_buffer, MAX_BUFFER, "Fifo msg %s != %s", msg_rcv.text,
                     WORDS[i]);
            fail(fifo_buffer);
            fifo_fail = 1;
        } else {
...
int main(void) {
    ...
    snprintf(buffer, MAX_LINE, "kill %d %d\n", SIGUSR1, signal_receiver_pid);
    ...
    printf(
        "%s[%d] Reading queue and fifo ... this may a take few seconds ...%s\n",
        BOLD, getpid(), UNBOLD);
