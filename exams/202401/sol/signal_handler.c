...
// signal flags
int xcpu_f = 0, ttou_f = 0, ttin_f = 0, usr1_f = 0, fpe_f = 0, xfsz_f = 0,
    usr2_f = 0, quit_f = 0, prof_f = 0, read_fifo = 0;
// sender pids
int usr1_pid, fpe_pid;

void signal_counter(int signu, siginfo_t *info,
...
void child_handler(int signu, __attribute__((unused)) siginfo_t *info,
                   __attribute__((unused)) void *ucontext) {
    if (signu == SIGQUIT) {
        quit_f = 1;
    }
