...
int fifoxxx_test(int success, char * string_input, int tmp_fd, enum program p) {
    ...
    char n_reader[2];
    if (success) sprintf(n_reader, "%d", len); 
    else sprintf(n_reader, "%d", len + 1);
